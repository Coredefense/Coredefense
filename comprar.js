...
planInputs.forEach(i=>{
  if(i.value===initialPlan){ i.checked=true; current=i }
  i.addEventListener('change',()=>{ current=i; update() })
})
if(!current && planInputs[0]){ planInputs[0].checked=true; current=planInputs[0] }
plus.addEventListener('click',()=>{ qty=Math.min(99,qty+1); update() })
